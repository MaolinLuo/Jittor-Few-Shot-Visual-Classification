...
class Evaluator:
    ...
    def __init__(self, cfg, animal_idxs, caltech_idxs, food_idxs, dog_idxs):
        self.cfg = cfg
        self.animal_idxs, self.caltech_idxs, self.food_idxs, self.dog_idxs = animal_idxs, caltech_idxs, food_idxs, dog_idxs
        self.reset()
    ...
    def process(self, mo, gt):
        ...
        pred = mo.argmax(1)[0]
        conf = nn.softmax(mo, dim=1).argmax(1)[1]  # Compute prediction confidences
        matches = pred.equal(gt).float()
        self._correct += int(matches.sum().item())
        self._total += gt.shape[0]

        self._y_true.extend(gt.data.tolist())
        self._y_pred.extend(pred.data.tolist())
        self._y_conf.extend(conf.data.tolist())
    ...
    def evaluate(self):
        ...
        # The first value will be returned by trainer.validate()
        ...
        results["Animal_acc"] = Animal_acc
        results["Caltech_acc"] = Caltech_acc
        results["Food_acc"] = Food_acc
        results["Thu_dog_acc"] = Thu_dog_acc
        print(f"* Animal: {Animal_acc:.1f}%  Caltech: {Caltech_acc:.1f}%  Food: {Food_acc:.1f}% Thu_dog: {Thu_dog_acc:.1f}%")
...
def compute_accuracy(output, target, topk=(1, )):
    ...
    Args:
        output: prediction matrix with shape (batch_size, num_classes).
