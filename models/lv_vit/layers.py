...
def to_2tuple(x):
    ...
    if isinstance(x, tuple) and len(x) == 2:
        return x
    raise TypeError("Invalid input for to_2tuple: input should be an int or a tuple of two ints.")
...
class GroupLinear(nn.Module):
    ...
    def __init__(self, in_planes, out_channels, groups=1, bias=True):
        ...
        self.groups = groups
        self.bias = bias
        self.group_in_dim = self.in_dim // self.groups
        self.group_out_dim = self.out_dim // self.groups

        self.group_weight = nn.Parameter(jt.zeros((self.groups, self.group_in_dim, self.group_out_dim)))
        if bias:
            self.group_bias = nn.Parameter(jt.zeros(self.out_dim))
        else:
            self.register_buffer('group_bias', None)

    def execute(self, x):
        t, b, d = x.shape
        x = x.reshape((t, b, self.groups, d // self.groups))
        out = jt.einsum('tbgd,gdf->tbgf', x, self.group_weight).reshape(t, b, self.out_dim)
        if self.bias:
            out += self.group_bias
        return out

    def extra_repr(self):
        s = f'{self.in_dim}, {self.out_dim}'
        if self.groups != 1:
...
class GroupNorm(nn.Module):
    def __init__(self, num_groups, embed_dim, eps=1e-5, affine=True):
        super().__init__()
        self.gn = nn.GroupNorm(num_groups, embed_dim, eps=eps, affine=affine)
...
class Attention(nn.Module):
    ...
    def execute(self, x, padding_mask=None):
        B, N, C = x.shape
        ...
        if padding_mask is not None:
            ...
            attn_float = attn.float32().softmax(dim=-1)
            attn = attn_float.astype(attn.dtype)
        else:
...
class FFNBlock(nn.Module):
    ...
    def flops(self, s):
        i = self.mlp_hidden_dim
        block_flops = dict(
            intermediate=self.dim * i,
            intermediate_act=ACTIVATION_FLOPS * i,
            intermediate_bias=i,
            output=self.dim * i,
            output_bias=self.dim,
            output_dropout=DROPOUT_FLOPS * self.dim,
            output_residual=self.dim,
            output_layer_norm=LAYER_NORM_FLOPS * self.dim,)
